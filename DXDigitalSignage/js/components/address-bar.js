browser.on("init", function () {
    ...
    this.navigateTo = loc => {
        ...
        //validate URL
        var isWebdev = loc.indexOf("en-us/microsoft-edge/tools/staticscan/") >= 0;
        var isTweetdev = loc.indexOf("twitter.com") >= 0;
        if(!isWebdev && !isTweetdev)
            loc = "https://dev.windows.com/en-us/microsoft-edge/tools/staticscan/";
    ...
    this.tweetIcon.addEventListener("click", () => {
        ...
        let tags = ["webdev", "lisbon"].map(encodeURIComponent);
        let text = encodeURIComponent(`Proving how awesome my website is at #mswebcamp using Microsoft Site Scan.`);
        let url = encodeURIComponent("http://aka.ms/testformodernweb");
